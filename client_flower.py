...
class ParkingFlowerClient(fl.client.NumPyClient):
    ...
    def _build_env(self):
        return MultiAgentParkingEnv(
            ...
            agents_json=self.env_config["agents_weighted_kmeans_balanced.json"],
            agent_name=self.agent_name,
            top_k=self.env_config.get("top_k", 5),
            max_steps=self.env_config.get("max_steps", 2500),
            agent_detection_radius=self.env_config.get("agent_detection_radius", 1500.0),
            use_gui=self.use_gui,
            warmup_steps=self.env_config.get("warmup_steps", 300),
            gui_delay=self.env_config.get("gui_delay", 0.02),
            parking_demand_prob=self.env_config.get("parking_demand_prob", 0.35),
        )
    ...
    def _normalize_reward(self, reward):
        self.reward_mean = 0.95 * self.reward_mean + 0.05 * reward
        self.reward_std = 0.95 * self.reward_std + 0.05 * (reward - self.reward_mean) ** 2
    ...
    def fit(self, parameters, config):
        ...
        steps_per_round = int(config.get("steps_per_round", 800))
    ...
    def evaluate(self, parameters, config):
        ...
        env = self._build_env()
        eval_steps = int(config.get("eval_steps", 120))
        warmup_steps = int(config.get("warmup_steps", 300))
